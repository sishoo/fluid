...
char **build_required_extensions(
    ...
    const uint32_t glfw_extensions_count,
    const char **glfw_extensions,
    const uint32_t requested_extensions_count,
...
#ifdef _DEBUG
    *final_extensions_buffer_ptr = VK_EXT_DEBUG_UTILS_EXTENSION_NAME;
...
bool check_validation_layer_support(const size_t num_validation_layers, const char *validation_layers[])
{
...
VkPhysicalDevice pick_best_physical_device(const uint32_t device_count, const VkPhysicalDevice *devices_buffer)
{
    if (device_count == 1)
    {
        VkPhysicalDevice device = devices_buffer[0];
    ...
    uint32_t best_device_heigharchy_value = 100;
    VkPhysicalDeviceProperties best_physical_device_properties;
    for (uint32_t i = 0; i < device_count; i++)
    {
        ...
        uint32_t device_type_heigharchy_value = device_type_heigharchy_table[device_properties.deviceType];
        if (device_type_heigharchy_value < best_device_heigharchy_value)
        {
            best_device = device;
            best_device_heigharchy_value = device_type_heigharchy_value;
            best_physical_device_properties = device_properties;
        }
    }
    printf("Using device '%s'.\n", best_physical_device_properties.deviceName);
    return best_device;
}
...
int main()
{
    ...
    */
    ...
    const size_t num_validation_layers = sizeof(validation_layers) / sizeof(validation_layers[0]);
    if (ENABLE_VALIDATION_LAYERS && !check_validation_layer_support(num_validation_layers, validation_layers))
    {
    ...
    physical_device = pick_best_physical_device(physical_device_count, physical_devices_buffer);
